...
設計書の検証項目をすべて実施し、**合格**。2026-08-04 に代表承認のうえ**本番反映済み**（下記「本番反映」）。
...
## 検収時に見つけた逸脱と修正（2件目・表示）

代表のステージング目視で、seed・night の「アプリの利用方法」手順番号 1〜3 が紺背景に紺数字（`styles.css` の `.app-guide-num` が `--color-primary`）で見えないと指摘。両ページのページ内 `<style>` に `color:var(--color-surface)`（白）を追加して修正（コミット `21848b6`、ステージングで白表示を確認済み）。

## 本番反映（2026-08-04）

- 経緯: 検収の途中で、`origin/main` がすでに実装コミット `c5f509f` まで進み本番デプロイ済みだったことが判明（別セッションによる反映）。つまり本番には「全曜日00:00–23:59」と「見えない手順番号」が公開されていた。今回の反映はその**是正**にあたる
- 手順: 代表承認のうえ、代表の手で `git push origin staging:main`（fast-forward、`21848b6` まで）→ `gh workflow run deploy.yml --ref main` を実行。Run `30895310114` success（28秒）
- 本番実測（デプロイ後）:
  - トップ JSON-LD: 平日 `opens 09:00 / closes 09:00`・週末 `00:00–23:59` — 設計書どおり
  - seed・night: `.app-guide-num` に `color:var(--color-surface)` あり — 手順番号は白表示
  - 本番URLに `x-robots-tag: noindex` なし（HTTP 200）— 検索対象のまま

## 残作業

なし（完了）。任意で、次回インデックス反映後に本番URLで Rich Results Test を再実行して登録状態を確認するとよい。
